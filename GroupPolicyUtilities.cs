...
{
    public  sealed class GroupPolicyUtilities
    {
        private GroupPolicyUtilities()
        {
        }

        private static void UpdateGroupPolicy()
        {
            var execFile = new FileInfo("gpupdate.exe");
            var proc = new Process
            {
                StartInfo =
                {
                    WindowStyle = ProcessWindowStyle.Hidden,
                    FileName = execFile.Name,
                    Arguments = "/force"
                }
            };
            proc.Start();
        ...
        private class Win32
        {
            public sealed class HandleKerberosTokenPurging
            {
                    ...
                    UInt32 kerberosPackageId)
                {
                    ...
                    {
                        if (purgeCacheRequestPtr != IntPtr.Zero) Marshal.FreeHGlobal(purgeCacheRequestPtr);
                        if (purgeCacheResponsePtr != IntPtr.Zero) OSCalls.LsaFreeReturnBuffer(purgeCacheResponsePtr);
                    }
                ...
                public void PurgeAll()
                {
                    ...
                    {
                        if (cacheRequestPtr != IntPtr.Zero)  Marshal.FreeHGlobal(cacheRequestPtr);

                        if (lsaHandle != IntPtr.Zero) OSCalls.LsaDeregisterLogonProcess(lsaHandle);
                    }
                }
            }
            ...
            internal class OSCalls
            {
                ...
                public static extern WinStatusCodes LsaLookupAuthenticationPackage([In] IntPtr LsaHandle,
                    [In] ref LSA_STRING PackageName, [Out] out UInt32 AuthenticationPackage);
        ...
        {
            new Win32.HandleKerberosTokenPurging()
                .PurgeAll();
